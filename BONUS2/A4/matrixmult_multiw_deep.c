 ...
 * Last modified date: December 7th
 * Creation date: December 7th
 **/
...
void sumRMatrix(int *childResult);
void writeRMatrix();
void zeroOut(int *matrix);
void printRMatrix();
void freeMem(char **matrix, int items);
void closeAllFDs(int fd[], int items);
void mmapDealloc(int *mmapArray[], int items, size_t size);
...
int getRMatrix(char *inputMatrix, char **wMatrices, const int numMatrices) {
    ...
    int fd[numMatrices];
    char **mmapFiles = (char**) malloc(numMatrices * sizeof(char *));
    int mmapFileNum = 0;
    int *childResult[numMatrices];
    ...
    for (int i = 0; i < numMatrices; ++i) {
        if (pipe(pipes[i]) == -1) {
            ...
            freeMem(mmapFiles, mmapFileNum);
            return 1;
        }

        char mmapFile[FN_SIZE * 2];
        snprintf(mmapFile, FN_SIZE * 2, "/tmp/mmap%d.tmp", ++mmapFileNum);
        if ((mmapFiles[i] = strdup(mmapFile)) == NULL) {
            fprintf(stderr, "Failed to allocate memory for mmap file name for child %d.\n", i);
            freeMem(mmapFiles, mmapFileNum);
            return 1;
        }
        

        fd[i] = open(mmapFile, O_RDWR | O_CREAT, 0644);
        if (fd[i] == -1) {
            fprintf(stderr, "Failed to create mmap temp file.\n");
            freeMem(mmapFiles, mmapFileNum);
            closeAllFDs(&fd[0], i);
            return 1;
        }

        if (ftruncate(fd[i], sizeof(int) * PRODUCT) == -1) {
        ...
        childResult[i] = mmap(NULL, sizeof(int) * PRODUCT, PROT_READ | PROT_WRITE, MAP_SHARED, fd[i], 0);
        if (childResult[i] == MAP_FAILED) {
            fprintf(stderr, "mmap() failed for child %d\n", i);
            freeMem(mmapFiles, mmapFileNum);
            closeAllFDs(&fd[0], i);
            return 1;
        }
    ...
    for (int i = 0; i < numMatrices; ++i) {
        ...
        if (pid < 0) {
            printf("fork() failed for child number %d.\n", i);
            freeMem(mmapFiles, mmapFileNum);
            mmapDealloc(childResult, numMatrices, sizeof(int) * PRODUCT);
            closeAllFDs(&fd[0], numMatrices);
            ...
            char mmapFD[FN_SIZE];
            snprintf(mmapFD, sizeof(mmapFD), "%d", fd[i]);

            // Prepare the arguments for matrixmult_parallel.
            char *args[] = {"matrixmult_parallel",
                inputMatrix, wMatrices[i], outFile, errFile, soutFDptr, mmapFD, NULL};

            // Call matrixmult_parallel.
            if (execv("./matrixmult_parallel.o", args) == -1) { // Error handling.
                fprintf(stderr, "execv() failed. Command tried to execute: %s %s %s %s %s %s %s\n", 
                        "./matrixmult_parallel.o", args[1], args[2], args[3], args[4], args[5], args[6]);
                close(outFD);
                close(errFD);
                closeAllFDs(&fd[0], numMatrices);
                freeMem(mmapFiles, mmapFileNum);
    ...
    for (int i = 0; i < numMatrices; ++i) {
        ...
        if (WIFEXITED(wstatus)) { // Check if the child process exited normally.
            ...
            if (exitStatus == 0) { // If the child process exited with code 0 (success)
                ...
                sumRMatrix(childResult[i]); // Sum up rSumMatrix and childResult matrices.
...
void mmapDealloc(int *mmapArray[], int items, size_t size) {
    for (int i = 0; i < items; i++) {
        if (munmap(mmapArray[i], size) == -1) {
            fprintf(stderr, "Failed to unmap memory for child %d.\n", i);
            return;
        }
    }
}
