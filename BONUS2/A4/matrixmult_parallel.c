...
int closeAllFiles(FILE *A, FILE *W, FILE *out_file, FILE *err_file);

int main(int argc, char *argv[]) {

    // Checks if there are exactly 7 arguments (4 files, 2 file descriptors and 1 default).
    if (argc != 7) {
        fprintf(stderr, "error: expecting exactly 6 inputs.\n");
    ...
    if (A == NULL || W == NULL || out_file == NULL || err_file == NULL) {
        ...
        fprintf(stderr, "Terminating, exit code 1.\n");
        return closeAllFiles(A, W, out_file, err_file);
    }
    ...
    int sharedMemFD = atoi(argv[6]);
    int *sharedResult = mmap(NULL, sizeof(int) * PRODUCT, PROT_READ | PROT_WRITE, MAP_SHARED, sharedMemFD, 0);
    if (sharedResult == MAP_FAILED) {
        fprintf(stderr, "Failed to open shared mmap region.\n");
        return closeAllFiles(A, W, out_file, err_file);
    }

    for (int  i = 0; i < PRODUCT; i++) {
        *(sharedResult + i) = *(&resultant[0][0] + i);
    }
 ...
 *                   arraytoFill: The resultant matrix to be filled.
 *
 * Returns: Nothing.
 **/
void fillRow(const int row, const int *sourceMatrix, int *arraytoFill) {
    const int rowStart = row * MAX_ROWS;

    for (int i = 0; i < MAX_COLUMNS; ++i) {
        *(arraytoFill + (rowStart + i)) = *(sourceMatrix + i);
    }
}
