...
void fillMatrix(double *matrix, int rows, int columns, FILE *file);
void rowSum(double *matrix1, double *matrix2, double *product, int row);
void fillRow(int row, double *sourceMatrix, double *resultant);
void printArr(double *matrix, int rows, int columns);
...
int main(int argc, char *argv[]) {
    ...
    // Calculate matrix multiplication of each row of the matrix in a different child process.
    for (int i = 0; i < MAX_PROCESSES; ++i) {
        ...
        if (pid == 0) { // If we are in the child process.
            ...
            // Calculate and store the row specific matrix multiplication.
    ...
    for (int i = 0; i < MAX_PROCESSES; ++i) {
        ...
        if (WIFEXITED(wstatus)) { // Check if the child process exited normally.
            ...
            if (exitStatus == 0) { // If the child process exited with code 0 (success)
                ...
                if (read(fd[i][0], rowResult, sizeof(double) * MAX_COLUMNS)
                    == -1) {
                    printf("Error while reading value. Problematic child: %d\n", 
                           childPID);
                    return 1;
...
/**
 * Calculates the matrix multiplication of the specified row in matrix1 with the entirety of 
 ...
 *                   product: Matrix in which the matrix multiplication will be stored. Must be of
 *                   size 1x8.
 *
 * Returns: Nothing.
**/ 
void rowSum(double* matrix1, double* matrix2, double* product, int row) {
    ...
    // Loop calculates the matrix multiplication of matrix1 and matrix2, and stores the result 
    // in product.
